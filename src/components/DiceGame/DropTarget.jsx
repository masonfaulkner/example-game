...
class DropTarget extends React.Component {
  ...
  componentDidMount() {
    this.elem.addEventListener(
      `${this.props.targetKey}DragEnter`,
      this.handleDragEnter,
      false
    );
    this.elem.addEventListener(
      `${this.props.targetKey}DragLeave`,
      this.handleDragLeave,
      false
    );
    this.elem.addEventListener(
      `${this.props.targetKey}Drop`,
      this.handleDrop,
      false
    );
    this.elem.addEventListener(
      `${this.props.targetKey}DisappearDraggedElement`,
      this.handleDisappearDraggedElement,
      false
    );
  }
  ...
  render() {
    const classNames =
      "droptarget " +
      (this.state.highlighted ? this.props.highlightClassName : "");
    return (
      <span
        ref={(t) => {
          this.elem = t;
